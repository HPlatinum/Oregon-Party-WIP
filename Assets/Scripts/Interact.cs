...
public class Interact : MonoBehaviour {
    ...
    public void Pickup() {
        ...
        inventory.AddItemToInventory(item, yield);
        // validates we can pick up item and adds it to the inventory
        bool attemptPickup = inventory.CanAddItemToInventory();
    ...
    // clears inventory on application quit
    ...
    public bool IsInteractAllowed() {
        ...
        if (interactSubject.interactType == Interactable.InteractTypes.Fishing) {
            if (true) { //check for required item (probably will be fishing rod)
                if (inventory.CanAddItemToInventory() && itemInHand == fishingRod) { //check if the player can carry the new item
                    return true;
                }
            }
        }
